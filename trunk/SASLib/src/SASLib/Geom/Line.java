 ...
 * @author Wil Cecil
 */
public class Line implements Shape, Cloneable, SimpleFunction {
     ...
     * then finds diffence between point and line
     * @param point
     * @param ignoreBounds Ignore Bounds?
     * @return double either positive, 0 or negative for GRTR THAN, On Line, LESS THAN
     * @throws java.lang.IllegalArgumentException if point.x is outside of segment bounds
     */
    public double compare(Point2D point, boolean ignoreBounds) throws IllegalArgumentException {
        ...
        double value;
        try {
            value = evaluate(point.getX()) - point.getY();
        } catch (Exception e) {
    ...
    /**
     * Checks is within the x bounds. This is is faster than Rectangle2D bound checking.
     * @param point
    ...
    /**
     * Checks is within the y bounds. This is is faster than Rectangle2D bound checking.
     * @param point
     * @return boolean true if x is within constraints
     */
    ...
    public Rectangle2D getBounds() {
        Rectangle2D boundline;

        boundline = new Rectangle2D.Double(pnt1.getX(), pnt1.getY(),0,0);
        boundline.add(pnt2.getX(), pnt2.getY());
    ...
    /**
     * Always fails, will throw UnsupportedOperationException always
     * @param f
