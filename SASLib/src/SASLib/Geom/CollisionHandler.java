...
public class CollisionHandler {
     ...
     */
    public synchronized static boolean checkGeneralCollision(Shape s1, Shape s2) throws IllegalArgumentException {
        ...
        if (s1 instanceof Circle) {
            ...
            if (s2 instanceof Quadrilateral) {
                return checkCollision((Quadrilateral) s2, (Line) s1);
            }
            ...
            if (s2 instanceof Line) {
                return checkCollision((Quadrilateral) s1, (Line) s2);
            }
        }
...
//            Rectangle2D r1 = Asub1.getBounds().union(Asub2.getBounds());
//            Rectangle2D r2 = Bsub1.getBounds().union(Bsub2.getBounds());
...
//            }
    ...
    public synchronized static boolean checkCollision(Line l1, Line l2) {
        ...
        if (SASLib.Header._DEBUG) {
            ...
            System.out.println("The X intersevtion value is " + x + " l1 y " +
                    l1.evaluate(x) + " l2 y " + l2.evaluate(x));
        }
     ...
     * @param l1 
     * @param l2 
     * @return the Point of collison, may be null if they never collide.
     */
    public synchronized static Point getCollision(Line l1, Line l2) {
        double y1, y2, x1, x2, m1, m2;
        ...
        if (SASLib.Header._DEBUG) {
            System.out.println("Values : y1 " + y1 + ", y2 " + y2 + ", x1 " + x1 + ", x2 " +
                    x2 + ", m1 " + m1 + ", m2 " + m2);
            System.out.println("The X intersevtion value is " + x + " l1 y " +
                    m1*(x-x1)+y1 + " l2 y " + m2*(x-x2)+y2);
        }

        if (l1.checkBounds(x) && l2.checkBounds(x)) {
            return new Point(x, m1*(x-x1)+y1);
        }
    ...
    public synchronized static boolean checkCollision(Triangle t, Line l) {
        ...
        // easy fail case
        //
        //Rectangle2D. Rectangle2D intersection

        Rectangle2D boundline = l.getBounds();
        ...
        if (Math.abs(delta) > Math.abs(Triangle._mindif)) {
            //Trinary Break Apart
            delta /= 3.0;
            Point p1 = new Point(l.pnt1.x + delta, l.evaluate(l.pnt1.x + delta));
            Point p2 = new Point(l.pnt1.x + delta * 2.0, l.evaluate(l.pnt1.x + delta * 2.0));
    ...
    /**
     * This method checks if a Quad is within or crosses a Quad
    ...
    /**
     * This method checks if a Triangle is within or crosses a Quad
     * @param q Quadrilateral 
     * @param l The Line
     * @return boolean true if intersection.
     */
    public synchronized static boolean checkCollision(Quadrilateral q, Line l) {
        //check bounding box
        if (!q.getBounds().intersects(l.getBounds())) {
            return false;
        }
        ...
        for (int i = 0; i < p1.length; i++) {
            //short circut sucess 
            //if ont line crosses then we have at least one point inside the other,
            if (checkCollision(new Line(p1[i], p1[(i + 1) % p1.length]), l)) {
                return true;
            }
        }

        //well hell we're here and there is no line corssing what if 
        //one quad is containing a line endpoint

        //QUAD check for Triangle vertex
        //this is last line so we can use return.
        return contains(q, l.getPnt1());
    }
    // </editor-fold>  
    // <editor-fold defaultstate="collapsed" desc=" Quadrilateral with Line ">  
    /**
    ...
    public synchronized static boolean contains(Triangle t, Point p) {
        ...
        for (int i = 0; i < t.lines.length; i++) {
            ...
            try {
                double y = t.lines[i].evaluate(p.x);
