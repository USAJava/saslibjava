 ...
 * y = (Tan(xScale * x - xShift) + yShift) / yScale
 * @author Wil Cecil
 */
public class Tan implements SimpleFunction{
     ...
     * @param yScale
     */
    public Tan( double xShift, double xScale, double yShift, double yScale){
        this.xScale = xScale;
        this.xShift = xShift;
        this.yScale = yScale;
        this.yShift = yShift;
    ...
    public Double evaluate(double dependent) {
        return (RotMath.Sine(xScale * dependent - xShift)/RotMath.CoSine(xScale * dependent - xShift) + yShift) / yScale;
    }
